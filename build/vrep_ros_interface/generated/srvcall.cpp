    ...
    else if(serviceClientProxy->serviceType == "plutodrone/PlutoPilot")
    {
        plutodrone::PlutoPilot srv;
        read__plutodrone__PlutoPilotRequest(p->stackID, &(srv.request), &(serviceClientProxy->rd_opt));
        if(serviceClientProxy->client.call(srv))
        {
            write__plutodrone__PlutoPilotResponse(srv.response, p->stackID, &(serviceClientProxy->wr_opt));
        }
        else
        {
            throw exception("failed to call service plutodrone/PlutoPilot");
        }
    }
